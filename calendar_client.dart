...
class CalendarClient {
  ...
  insert(title, startTime, endTime) {
    var _clientID = new ClientId(
        "232616029192-e6q0rc4jhkp5pbaacil5fkfb7ik7nrrj.apps.googleusercontent.com",
        "");
    clientViaUserConsent(
            _clientID, _scopes, prompt('', title, startTime, endTime))
      ...
      try {
        calendar.events.insert(event, calendarId).then((value) {
          log("ADDEDDD_________________${value.status}");
  ...
  prompt(String url, title, startTime, endTime) async {
    log("Please go to the following URL and grant access:");
    log("  => $url");

    // ignore: deprecated_member_use
    if (await canLaunch(
        'https://calendar.google.com/calendar/u/0/r/create?dates=$startTime/$endTime&title=$title&details= hy this Is My ook App Please Follow this Instractions and remeained me after 1 hour')) {
      // ignore: deprecated_member_use
      await launch(
          'https://calendar.google.com/calendar/u/0/r/create?dates=$startTime/$endTime&title=$title&details= hy this Is My ook App Please Follow this Instractions and remeained me after 1 hour');
    } else {
